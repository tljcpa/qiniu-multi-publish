...
公网 demo 的写接口无鉴权，调用 DeepSeek/Azure 的端点若被脚本刷会直接消耗
LLM 余额。这里做一个进程内滑动窗口限流，挡住明显的滥刷，
对正常人类操作（每分钟几次）无感。与 voice / pr-review 两项目同款实现。
...
# 调用 LLM 的花费型端点：每 IP 每分钟 30 次（人类够用，挡脚本滥刷）。
